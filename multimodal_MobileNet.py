...
for subset in range(0,5):
    ...
    base_MobileNet=MobileNet(weights='imagenet', include_top=False, input_shape=(img_rows, img_cols, img_channel))
    
    output_shared = base_MobileNet(x)
    ...
    model_all.save('MTL_MobileNet_clinical_%d.h5' % subset)
    ...
    f = open('results_MTL_MobileNet_clinical.txt', 'a+')
    f.write('\n\nNCI_%d:' % subset +
